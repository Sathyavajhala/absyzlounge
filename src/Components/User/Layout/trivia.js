...
class Trivia extends Component {
    ...
    engagementratio() {
        const requestOptions = {
            method: "GET",
            headers: { token: `${this.props.getprops.match.params.name}`, 'Content-Type': 'application/json', 'username': '' }
        }
    ...
    fetchQuestion(initialLoad) {
        console.log('question', `${this.props.getprops.match.params.name}`)
        const requestOptions = {
            method: 'GET',
            headers: { 'token': `${this.props.getprops.match.params.name}`, 'Content-Type': 'application/json' }
        }
        fetch(`${api}/employeeportal/nextquestion`, requestOptions)
            ...
            .then(
                (result) => {
                    ...
                    console.log('result ------------>', result.payload.username, result.payload.email)
                    const name = result.payload.username;
                    const email = result.payload.email;
                    if (result.statusCode === 500) {
                        this.fetchQuestion()
                    } else if (result.payload.msg == "all questions answered successfully") {
                        if (!initialLoad) {
    ...
    handleNext() {
        ...
        if (this.state.answer == null || this.state.answer == '') {
            ...
            const requestOptions = {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', token: `${this.props.getprops.match.params.name}` },
                body: JSON.stringify({
    ...
    render() {
        ...
        console.log(this.props.getprops.match.params.name, "my props")
            ...
            <div>
                {this.state.myQuestionView  ?
                    <div>
                        ...
                        :
                        <div
                        style={myStyles.mobileAppBarDiv}
                    >
                        <p
                            style={myStyles.mobileAppBarText}
...
export default windowSize(Trivia)
const myStyles = {
    desktopAppBarDiv: {
        width: '100%',
        position: 'fixed',
